...
function loadData() {
    ...
    if (!users.find(u => u.username === "demo")) {
        users.push({ username: "demo", password: "123", fullname: "Demo User", phone: "0123456789", email: "demo@yumexpress.com", address: "", addresses: [] });
    }
    updateCartCount();
}
...
function renderCartSidebar() {
    ...
    document.getElementById("cartTotal").innerHTML = `Tổng: ${total.toLocaleString()}đ`;
...
function checkoutHandler() {
    ...
    let paymentMethod = confirm("Chọn COD (OK) hay Online (Cancel)?") ? "COD" : "Online";
    let newOrder = {
        id: Date.now(),
    ...
    renderCartSidebar();
    saveToLocal();
    showToast(`Đặt hàng thành công! Phương thức: ${paymentMethod}`);
...
function handleRegister() {
    ...
    const newUser = { username: username, password: password, fullname: fullname, phone: phone, email: email, address: address, addresses: address ? [address] : [] };
    users.push(newUser);
    currentUser = newUser;
    saveToLocal();
...
// ==========================================
// ĐÃ CẬP NHẬT: TRANG TIN TỨC
// ==========================================
function renderNews() { 
    const articles = [
        {
            title: "Siêu hội Pizza: Mua 1 Tặng 1 duy nhất vào ngày thứ Tư",
            date: "28/05/2026",
            img: "https://images.unsplash.com/photo-1513104890138-7c749659a591?w=500",
            summary: "Tin vui cho các tín đồ đạo Pizza! Duy nhất thứ tư tuần này, khi đặt bất kỳ Pizza size L tại YumExpress, bạn sẽ được tặng ngay một Pizza đồng size..."
        },
        {
            title: "YumExpress chính thức ra mắt dịch vụ giao hàng 'Siêu tốc 15 phút'",
            date: "25/05/2026",
            img: "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=500",
            summary: "Nhờ tối ưu hóa thuật toán định tuyến và mở rộng đội ngũ tài xế công nghệ, YumExpress cam kết các đơn hàng trong bán kính 3km sẽ được giao chỉ trong 15 phút."
        },
        {
            title: "Mách bạn 5 món ăn giải nhiệt cực đỉnh cho mùa hè oi bức",
            date: "20/05/2026",
            img: "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=500",
            summary: "Nắng nóng đỉnh điểm khiến bạn mệt mỏi? Hãy cùng YumExpress điểm qua danh sách 5 món salad và sinh tố thanh mát giúp thanh lọc cơ thể ngay tức thì nhé!"
        }
    ];
...
function renderAdminPanel(tab) {
    ...
    } else if (tab === "users") { container.innerHTML = `<h2>Người dùng</h2><ul style="background:white;padding:20px;border-radius:16px;">${users.map(u => `<li>👤 ${u.fullname || u.username} - ${u.email} - ${u.phone || ''}</li>`).join("")}</ul>`; }
