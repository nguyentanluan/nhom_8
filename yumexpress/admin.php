    ...
    <table>
        ...
        <tbody>
            <?php while($row = $result->fetch_assoc()): 
                // 1. Tính toán giá sau khi giảm (nếu có cột discount trong DB)
                $original_price = $row['price'];
                $discount = isset($row['discount']) ? intval($row['discount']) : 0;
                $current_price = $original_price - ($original_price * $discount / 100);

                // 2. Xử lý hiển thị màu sắc và chữ cho 3 trạng thái tuyển chọn
                $status_html = '';
                switch ($row['status']) {
                    case 1:
                        $status_html = '<span style="color: #28a745; font-weight: 600;"><i class="fas fa-check-circle"></i> Đang bán</span>';
                        break;
                    case 2:
                        $status_html = '<span style="color: #ffc107; font-weight: 600;"><i class="fas fa-exclamation-triangle"></i> Gần hết hàng</span>';
                        break;
                    case 3:
                        $status_html = '<span style="color: #dc3545; font-weight: 600;"><i class="fas fa-times-circle"></i> Hết hàng</span>';
                        break;
                    default:
                        $status_html = '<span style="color: #6c757d;">Ngừng kinh doanh</span>';
                        break;
                }
            ?>
            <tr>
                ...
                <td>
                    <?php if ($discount > 0): ?>
                        <span style="text-decoration: line-through; color: #999; font-size: 0.9rem;"><?php echo number_format($original_price, 0, ',', '.'); ?>đ</span>
                        <br>
                        <span style="color: #e67e22; font-weight: bold;"><?php echo number_format($current_price, 0, ',', '.'); ?>đ</span>
                        <span style="background: #ffe3e3; color: #dc3545; padding: 2px 6px; border-radius: 4px; font-size: 0.8rem; font-weight: bold; margin-left: 5px;">-<?php echo $discount; ?>%</span>
                    <?php else: ?>
                        <span style="font-weight: bold;"><?php echo number_format($original_price, 0, ',', '.'); ?>đ</span>
                    <?php endif; ?>
                </td>
                <td>
